...
class ZIOInterpreter(printer: Printer) {
  private def eval(datum: Expression): Op[Value] =
    datum match {
      ...
      case Lambda(formals, body, _)           => obLambda(formals, body)
      case Quote(v)                           => opDatum(v)
      case Begin(exprs)                       => opSequence(exprs)
      case If(test, consequent, alternative)  => opIf(test, consequent, alternative)
      case Assign(Variable(sym), value)       => opSet(Value.Symbol(sym.value), value)
  ...
  @inline private def opError[T](kind: String, message: String, location: Option[SourceInformation]): Op[T] =
  ...
  private def obLambda(
    ...
    case FixedArity(args) => opValue(Procedure(args.map(x => Value.Symbol(x.value.value)), eval(Begin(body))))
...
object ZIOInterpreter {
  ...
  def interpret[T](source: String, context: Context): ZIO[T, Throwable, Value] =
    for {
      exprs  <- ZIO.fromTry(parser.parse(source))
